...
def get_section_data(data, section_name):
    """
...
def get_possible_alphabet(data):
    ...
    rdata_data = get_section_data(data, b".rdata")
...
def get_profile_section_contents(data):
    ...
    return get_section_data(data, b".profile")
...
def extract_config(data):
    ...
    # Nighthawk DLLs are known to contain a .profile section which contains
    # configuration information.
    profile_section_contents = get_section_data(data, b".profile")
    ...
    for plaintext_alphabet, ciphertext_alphabet in possible_alphabets:
        ...
        if "implant-config" in config_:
            # This is a heuristic and may fail. I observed this in just the
            # one sample that I had and was available
...
def main():
    ...
    parser = argparse.ArgumentParser("Nighthawk DLL Configuration Extractor")
    parser.add_argument("--fpath", "-f", type=str, required=True,
                        help="Path to Nighthawk DLL")
    ...
    if data is None:
        print("[-] Couldn't read file contents")
        return 1
